...
msgpack_numpy.patch()
def get_id(fname):
    r1 = re.search(r'_[0-9][0-9]+.npz',fname)
...
class DetectFeatLmdb(object):
    def __init__(self, img_dir, conf_th=0.2, max_bb=100, min_bb=10, num_bb=36,
        ...
        self.img_dir = img_dir  # added 
        self.min_bb = min_bb# added 
        self.max_bb = max_bb# added 
        self.num_bb =num_bb # added 
        self.conf_th = conf_th # added 
        if conf_th == -1:
            ...
            if not exists(f'{img_dir}/{nbb}'):
                ...
                #modifications
                self.name2nbb = {}
                a = json.load(open(f'{img_dir}/{nbb}'))
                for k , d in a.items():

                    self.name2nbb[get_id(k)+'.npz']=d
        ...
        print(self.txn.stat()['entries'])
        if self.name2nbb is None:
            self.name2nbb = self._compute_nbb()
    def _compute_nbb(self):
        name2nbb = {}
        fnames = json.loads(self.txn.get(key=b'__keys__').tobytes()) # changed from decode(utf-8) to tobytes()
    ...
    def __getitem__(self, file_name):
        ...
        if file_name.startswith('0i'):
            #print(file_name)
            dump = self.txn.get(file_name[1:].encode('utf-8'))
            nbb = self.name2nbb[file_name[7:]]
        elif file_name[0] !='c':
            fn = ('flickr30k_'+file_name)
            dump = self.txn.get(fn.encode('utf-8'))
            nbb = self.name2nbb[file_name]

        else :
            dump = self.txn.get(file_name.encode('utf-8'))
            nbb = self.name2nbb[get_id(file_name)+'.npz']
...
class DetectFeatTxtTokDataset(Dataset):
    def __init__(self, txt_db, img_db,bw=False):
        ...
        self.lens = [tl + 1#self.img_db.name2nbb[get_id("0"+txt2img[id_])+".npz".lower()]
