...
captions = [(data[k]['caption'],data[k]['alternative_caption'],data[k]['replacement'])  for k in data.keys() ]
...
def make_change(inputt,toker,data,device):
    tokenizer = toker
    t =tokenToString(" ".join(toker.convert_ids_to_tokens(inputt[0,:].tolist())))
    changed = False
    target = 0
    results= []
    for caption,alter_cap,rep in data:
        if tokenToString(caption) == t and alter_cap not in caps:
            print("text : ",t)

            print("alter : ",alter_cap)
            try :
                repp = [toker.convert_tokens_to_ids([a]) for a in rep]
                print("remplacement : ",rep,repp )
            except KeyError:
                print("###################")
                return results
            ...
            idx = (inputt == repp[0][0]).nonzero()
            print("idx",idx)
            #print(inputt[idx[0][0].item(),idx[0][1].item()] )
            #print(repp)
            if idx.tolist() != []:
                inputt[idx[0][0].item(),idx[0][1].item()] = repp[1][0]
                print(inputt)
                changed = True
                target = 1
                results.append((inputt.to(device),target,rep))
    return results 

def make_change1(inputt,toker,data,device):

    tokenizer = toker
    t =tokenToString(" ".join(toker.convert_ids_to_tokens(inputt.tolist()[0])))
    changed = False
    target = 0
    results = []
    for caption,alter_cap,rep in data:
...
def del_PAD(text):
  return [token for token in text.split(" ") if token !="[PAD]"]
class LXMERT:
    ...
    def get_representation(self,eval_tuple: DataTuple,id2nb):
        print("model =>",args.model, "dec ?", args.dec)
        self.model.eval()
        device =torch.device("cuda")
        eval_ld = eval_tuple.loader
        polled ,visual ,lang, fnames ,texts,changed_text,targets,remplacements= [],[],[],[],[],[],[],[]
        targets,shifted_idx = [],[] 
        print(f"len = {len(eval_ld)}")
        for m in [make_change1,make_change]:
         if args.dec ==1:
            eval_ld.dataset.reload()
        ...
        # language Inputs
                ...
                results = m(input_ids,self.tokenizer,captions,device)
                if results != [] :
                  for (input_idss,target,rep) in results :
                    ...
                    with torch.no_grad():        
                        ...
                        input_idss, segment_ids, input_mask,
                    ...
                    changed_text.append(" ".join(self.tokenizer.convert_ids_to_tokens(input_idss.tolist()[0])))
                    targets.append(target)
                    remplacements.append(rep)
                    del lang_output
                    del visn_output
                    del pooled_output
        ...
        repre = {"fnames":fnames , "texts":texts ,'visual':visual,'pooled_representation':polled,"lang":lang,
                "targets":targets,"changed_text":changed_text,"remplacements":remplacements}
        FILE = '../representations/lxmert_altercap_'+args.model+'_coco.pickle' if args.dec == 0 else '../representations/lxmert_altercap_'+args.model+'coco_dec.pickle'
        print("saving :",FILE)
...
if __name__ == "__main__":
    ...
    lxmert.get_representation(valid_tuple,id2nb)
