...
class LXMERT:
    ...
    def __init__(self, max_seq_length):
        ...
        if args.model =="vqa":
            ...
            self.inf_model = self.model.lxrt_encoder.model.bert
        self.color_idx  = self.tokenizer.convert_tokens_to_ids(color_words)
    ...
    def get_representation(self,eval_tuple: DataTuple):
        ...
        # language Inputs
                ...
                if len(set.intersection(set(text.split(" ")),set(color_words))):
                    
                    
                    input_ids_,color,idx = MASK_COLOR(input_ids.squeeze(0),self.color_idx,mask=self.tokenizer.convert_tokens_to_ids(['[MASK]'])[0])
                    masked = " ".join(self.tokenizer.convert_ids_to_tokens(input_ids_.tolist()))

                    #print("#",target," ".join(self.tokenizer.convert_ids_to_tokens(colors)))
                    ...
                    targets.append((color,self.tokenizer.convert_ids_to_tokens([color])))
                    bert_outputs.append(bert_output[:,idx,:].to("cpu"))
                    del lang_output
                    del visn_output
                    del pooled_output
                    torch.cuda.empty_cache()

        repre = {"fnames":fnames , "texts":texts ,'visual':visual,'masked_rep':polled,"lang":lang,
                "masked_sentences":masked_sentences,args.task:targets,args.task+"_cls":[self.color_idx,color_words]}

        filename = "../representations/lxmert_"+args.task+"_"+args.model+"_"+args.data+".pickle" if args.dec==0 else "../representations/lxmert_"+args.task+"_"+args.model+"_"+args.data+"_dec.pickle"
