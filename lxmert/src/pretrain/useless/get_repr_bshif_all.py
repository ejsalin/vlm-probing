...
class LXMERT:
    ...
    def get_representation(self,eval_tuple: DataTuple,opts):
        ...
        # language Inputs
                ...
                length =  len(del_PAD(text).split(" "))
                if tokenToString(text) in data_.keys():
                    a = data_[tokenToString(text)]
                    input_ids,target,shift_id = shift_(input_ids,length,a[1],a[2])
                    nb +=1
                else :
                    input_ids,target,shift_id = shift(input_ids,length)
        ...
        print("nombre d'examples :",nb)
        repre = {"fnames":fnames , "texts":texts ,'visual':visual,'pooled_representation':polled,"lang":lang,
                "input_ids":input_idss,"targets":targets}
        filename = "../representations/lxmert_bshift_"+args.model+"_"+args.data+".pickle" if args.dec==0 else "../representations/lxmert_bshift_"+args.model+"_"+args.data+"_dec.pickle"
