...
Split2ImgFeatPath = {
    ...
    'mscoco_minival_bw': 'data/coco_val_bw/train_obj36.tsv'
...
class LXMERTDataset:
    def __init__(self, splits: str, qa_sets=None):
        ...
        for source in self.sources:
            print("data/lxmert/%s.json" % source)
...
class LXMERTTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        ...
        used_data = []
        print(list(self.imgid2img.keys())[:5])
        a = input(">")
        ...
        for datum in used_data:
            sentf = datum['sentf']
            for sent_idx,sent in enumerate(sentf["mscoco"]):
                    new_datum = {
                        'uid': make_uid(datum['img_id'], "mscoco", sent_idx),
                        'img_id': datum['img_id'],
                        'sent': sent
                    }
    ...
    def __getitem__(self, item: int):
        ...
        example = InputExample(
            ...
            is_matched, label,image_id=img_id #img_id added 
        )
        return example
