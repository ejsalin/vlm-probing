...
text_data = {
    'flickr': 'data/flickrfeatures/results.csv',
    'flickr_bw': 'data/flickr_bw_features/train_obj36.tsv',
    ...
    def __init__(self, splits: str, qa_sets=None):
        ...
        with open(text_data[args.valid]) as csvfile:
            ...
            for line in tqdm(ann, desc='processing flickr'):
                if args.valid=="flowers":
                    self.data.append((line[0][:-7],' '.join(line[2:])))
                else:
                    self.data.append((line[0][:-5],' '.join(line[2:])))
...
class LXMERTTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        ...
        used_data = []
        print(list(self.imgid2img.keys())[:10])
    ...
    def __getitem__(self, item: int):
        ...
        img_info = self.imgid2img[img_id]
        #print(img_info['num_boxes'])
        #print(img_info['features'])
        #print(img_info['boxes'])
        #print(img_info['objects_id'])
        #print(img_info['objects_conf'])
        #print(img_info['attrs_id'])
        #print(img_info['attrs_conf'])
        #a = input()
        ...
        example = InputExample(
            ...
            is_matched, label,image_id=(datum['img_id'])  #img_id added 
        )
        return example
...
class LXMERTTorchDataset_deccorellated(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1,seed=0):
        super().__init__()
        self.raw_dataset = dataset
        self.task_matched = False
        self.used_datum = []
    ...
    def __getitem__(self, item: int):
        ...
        other_datum = random.sample(self.data_bis ,1)[0]
        while other_datum['img_id'] == img_id:
            other_datum =random.sample(self.data_bis,1)[0]

        self.data_bis.remove(other_datum)
        ...
        example = InputExample(
            uid, sent, (feats, boxes),
            (obj_labels, obj_confs), (attr_labels, attr_confs),
            is_matched, label,image_id=(img_id,other_datum['img_id'])  #img_id added 
        )
        return example