...
import scipy.io
mat = scipy.io.loadmat('./split/imagelabels.mat')
print(mat.keys())
...
split = scipy.io.loadmat('./split/setid.mat')
print(split['trnid'])
...
def main(args):
  ...
  # LXMERT DATA
    ...
    for ex_fname,title in zip([split['trnid'][0],split['tstid'][0]],['train','val']):
      ...
      fnames, pooled_representation, targets = [],[],[]
      added_fnames = []


      for fnames_, pooled_representation_,targets_  in tqdm(zip(lxmert_data['fnames'], lxmert_data['pooled_representation'],  lxmert_data['targets']),total=len(lxmert_data['targets'])):
      ...
      data_lxmert[title] = {'fnames':fnames, 'pooled_representation':pooled_representation,  'targets':targets}
  ...
  # vit DATA
    ...
    for ex_fname,title in zip([split['trnid'][0],split['tstid'][0]],['train','val']):
      ...
      for fnames_, pooled_representation_,targets_  in tqdm(zip(vit_data['fnames'], vit_data['pooled_representations'],  vit_data['nobj']),total=len(vit_data['nobj'])):
      ...
      data_vit[title] = {'fnames':fnames, 'pooled_representations':pooled_representation,  'targets':targets}
  ...
  # resnet DATA
    ...
    for ex_fname,title in zip([split['trnid'][0],split['tstid'][0]],['train','val']):
      ...
      for fnames_, pooled_representation_,targets_  in tqdm(zip(resnet_data['fnames'], resnet_data['representations'],  resnet_data['nobj']),total=len(resnet_data['nobj'])):
      ...
      data_resnet[title] = {'fnames':fnames, 'representations':pooled_representation,  'targets':targets}
    ...
    c = Counter(data_lxmert['train']['targets'])
    print(len(c))
    print([(i, c[i] / len(data_lxmert['train']['targets']) * 100.0) for i, count in c.most_common(10)])

    print(Counter(data_lxmert['train']['targets']))
